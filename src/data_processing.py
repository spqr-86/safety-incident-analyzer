...
from langchain_community.document_loaders import PyPDFDirectoryLoader # <-- ПРАВИЛЬНЫЙ ЗАГРУЗЧИК
...
def load_and_chunk_documents(directory_path: str) -> List[Document]:
    ...
    try:
        ...
        if not documents:
            print(f"Предупреждение: В директории {directory_path} не найдено PDF-файлов.")
            return []
