...
def create_hyde_rag_chain():
    ...
    hyde_prompt = ChatPromptTemplate.from_template(
        "Напиши короткий, гипотетический ответ на следующий вопрос, как если бы он был взят из официального документа по охране труда: {question}"
    )
    ...
    # 4. Финальный промпт
    final_template = """Вы — ИИ-ассистент. Используя ТОЛЬКО предоставленный Контекст, дайте четкий и исчерпывающий ответ на Вопрос. Не придумывайте информацию. Отвечай на русском языке.
...
Ответ:"""
    ...
    hyde_rag_chain = (
        ...
        | RunnablePassthrough.assign(
              ...
              context=itemgetter("question") | hyde_chain | compression_retriever
          )
        | RunnablePassthrough.assign(
