...
def create_final_rag_chain():
    ...
    condense_question_template = """Учитывая историю диалога и новый вопрос,
    переформулируй новый вопрос так, чтобы он был самостоятельным.
    ...
    answer_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Ты — вежливый и полезный AI-ассистент по нормативной документации. Отвечай на вопрос, основываясь ТОЛЬКО на следующем контексте. Если информации в контексте недостаточно, скажи, что не знаешь ответа.\n\nКонтекст:\n{context}",
            ),
    ...
    final_chain = {
        "context": RunnableBranch(
            (
                ...
                standalone_question_chain,
            ),  # ЕСЛИ история есть, ТО запустить переформулировку
