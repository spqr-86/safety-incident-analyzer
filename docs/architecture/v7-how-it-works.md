...
```python
# bridge.py
visual_proof_fn = make_visual_proof_fn(vision_llm)
inject_visual_proof_fn(visual_proof_fn)   # → set_visual_proof_fn() в ноде
```

На VPS нет `agent_tools` (VLM не настроен) → нода пропускается без ошибок.
