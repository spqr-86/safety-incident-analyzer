...
4.  **Evaluation Framework**: Встроенная система оценки качества (RAGAS, кастомные метрики) для контроля деградации при изменениях.
...
- 🚀 [Quick Start: Evaluation & Metrics](./QUICK_START.md) — руководство по системе оценки качества.
- 📖 [План развития](./DEVELOPMENT_PLAN.md) — дорожная карта проекта.
- 📊 [Примеры использования Evaluation](./eval/EXAMPLES.md) — примеры работы системы оценки.
- 🧪 [Benchmarks и Baseline Метрики](./benchmarks/README.md) — результаты тестирования производительности.

### Для разработчиков (technical docs)
- [Анализ кодовой базы](./docs/CODEBASE_ANALYSIS.md)

### Быстрая диагностика проблем
- [Гайд по тестированию документации](./docs/TESTING_DOCS.md)
...
flowchart LR
    subgraph Ingestion[Индексация]
        Docs[Документы] --> Docling[Docling -> MD]
    ...
    subgraph Retrieval[Поиск]
        Query[Вопрос] --> Hybrid[Векторы + BM25]
        Hybrid --> Rerank[FlashRank]
        Rerank --> Context[Топ-контекст]
    end

    subgraph Agents[LangGraph Agents]
        Context --> RC[Relevance Checker]
        RC -->|Да| RA[Research Agent]
        RC -->|Нет| Stop[Отказ]
        RA --> VA[Verification Agent]
        VA -->|Ок| Answer[Ответ]
        VA -->|Исправить| RA
    end
```
