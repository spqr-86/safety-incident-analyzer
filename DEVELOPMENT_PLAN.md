...
- Метрики для retrieval качества (NDCG, MRR, Hit Rate)
- Автоматические детекторы галлюцинаций
- Мониторинг production метрик
- Систематическое версионирование экспериментов
- Расширенный датасет (16 вопросов недостаточно)
- Анализ failure cases и edge cases
...
1. ✅ Retrieval метрики (`src/retrieval_metrics.py`)
2. ✅ Generation метрики (`src/generation_metrics.py`)
3. ✅ Детектор галлюцинаций
4. ✅ Расширение датасета до 50 вопросов

**Deliverables:**
- Working eval pipeline с 10+ метриками
- Baseline замеры для текущей системы
- Отчет с анализом baseline

---

### **Sprint 2 (1-2 недели): Автоматизация**
1. ✅ Интеграция W&B для experiment tracking
2. ✅ CI/CD pipeline (GitHub Actions)
3. ✅ Расширение датасета до 100 вопросов
4. ✅ Retrieval dataset с аннотацией релевантности

**Deliverables:**
- Автоматический eval при каждом PR
- W&B дашборды с метриками
- Расширенный тестовый датасет

---

### **Sprint 3 (1-2 недели): Production и анализ**
1. ✅ Production мониторинг в Streamlit
2. ✅ Drift detection
3. ✅ Error analysis и failure cases
...
- [ ] Retrieval метрики реализованы и stable
- [ ] Generation метрики показывают > 80% correctness
- [ ] Golden dataset > 100 вопросов
- [ ] CI/CD eval pipeline работает
- [ ] Production мониторинг настроен
- [ ] Drift detection активен
- [ ] Error analysis проводится регулярно
- [ ] Документация по eval обновлена
...
1. ✅ Создать `src/retrieval_metrics.py` с базовыми метриками (Hit Rate, MRR)
2. ✅ Расширить `src/generation_metrics.py` (добавить faithfulness, relevance)
3. ✅ Генерация 30 дополнительных вопросов для датасета
4. ✅ Запустить baseline eval и сохранить результаты

### Краткосрочно (этот месяц):
1. Настроить W&B или MLflow для tracking
2. Довести датасет до 100 вопросов
3. Реализовать автоматический детектор галлюцинаций
4. Создать GitHub Action для eval

### Долгосрочно (квартал):
1. Production мониторинг с дашбордами
2. Hyperparameter optimization
3. A/B testing infrastructure
4. Edge cases coverage > 80%
